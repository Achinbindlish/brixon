        ...
        <div className="bg-card rounded-xl border border-border p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <input
              ...
              placeholder="Article #"
              className="h-10 px-3 rounded-lg border border-input bg-card text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
            />
            <input
            ...
            />
            <input
              value={newArticle.stock_unit}
              onChange={(e) => setNewArticle({ ...newArticle, stock_unit: e.target.value })}
              placeholder="Stock unit (e.g. meter)"
              className="h-10 px-3 rounded-lg border border-input bg-card text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
            />
          </div>
          <button
            onClick={handleAddArticle}
            disabled={upsertArticle.isPending}
            className="h-10 px-6 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:opacity-90 disabled:opacity-50 transition-all"
          >
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {articles?.map((a) => {
                ...
                const isEditing = editingStock[a.id] !== undefined;
                return (
                  <tr key={a.id} className="border-b border-border last:border-0">
                    <td className="px-4 py-3 text-foreground font-medium">{a.article_number}</td>
                    <td className="px-4 py-3 text-foreground">{a.description}</td>
                    <td className="px-4 py-3 text-right text-foreground">₹{a.price?.toLocaleString("en-IN")}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <input
                          type="number"
                          min="0"
                          value={isEditing ? editingStock[a.id] : ""}
                          onChange={(e) => setEditingStock({ ...editingStock, [a.id]: e.target.value })}
                          placeholder={String(stock)}
                          className="w-20 h-8 px-2 rounded-lg border border-input bg-card text-foreground text-right text-xs focus:outline-none focus:ring-2 focus:ring-ring"
                        />
                        {isEditing && (
                          <button
                            onClick={() => handleStockUpdate(a.id)}
