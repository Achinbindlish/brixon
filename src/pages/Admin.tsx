  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="w-full px-4 sm:px-6 py-4 sm:py-6 space-y-4 sm:space-y-5 max-w-5xl mx-auto">
        <div className="flex border border-border rounded-md overflow-hidden overflow-x-auto">
          {tabs.map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className={`flex-1 min-w-0 h-9 text-xs sm:text-sm font-medium flex items-center justify-center gap-1 sm:gap-1.5 transition-colors ${
